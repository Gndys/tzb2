...
export default defineEventHandler(async (event) => {
  ...
  const isSubscription = query.subscription === 'true'
  const config = (await import('@config')).config

  if (!orderId) {
    return sendRedirect(event, `${config.app.payment.cancelUrl}?provider=paypal`)
  }
  ...
  if (isSubscription) {
    try {
      ...
      if (!orderRecord?.providerOrderId) {
        return sendRedirect(event, `${config.app.payment.cancelUrl}?provider=paypal&order_id=${orderId}`)
      }
