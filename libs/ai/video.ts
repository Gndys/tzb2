...
/**
 * Build provider request payload preview for debugging and local verification.
 * This does not send network requests and intentionally excludes auth headers.
 */
export function buildVideoProviderRequestPayload(options: VideoGenerationOptions): Record<string, unknown> {
  ...
  if (provider === 'fal') {
    ...
    return {
      provider: 'fal',
      model,
      prompt: firstFrameUrl ? { text: options.prompt, image: firstFrameUrl } : options.prompt,
      aspectRatio: options.aspectRatio,
      seed: options.seed,
      providerOptions: {
        ...(duration !== undefined ? { duration } : {}),
        ...(options.loop !== undefined ? { loop: options.loop } : {}),
        ...(options.motionStrength !== undefined ? { motionStrength: options.motionStrength } : {}),
        pollTimeoutMs: DEFAULT_MAX_TIMEOUT_MS,
      },
    };
  }
  ...
  if (provider === 'volcengine') {
    ...
    return {
      ...
      model,
      content: buildVolcengineContent(options),
      ratio: getVolcengineRatio(options),
      resolution: getVolcengineResolution(options, model),
      duration: options.duration,
      seed: options.seed,
      watermark: options.watermark ?? false,
    };
